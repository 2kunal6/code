...
int main() {
	int hh,mm,h,d,c,n;
	cin>>hh>>mm>>h>>d>>c>>n;
	double wu=(ceil((double)h/n))*c;
	//cout<<h/n<<" "<<ceil(h/n)<<" "<<c<<" "<<endl;
	int wait=0;
	if(hh<20) {
		if(mm==0)wait=(20-hh)*60;
		else wait=(20-hh-1)*60+(60-mm);
	}
	//cout<<wait<<endl;
	double dis=(ceil((h+(d*wait))/(double)n))*.8*c;
	//cout<<dis<<endl;
	cout<<min(wu,dis)<<endl;
	return 0;
}
